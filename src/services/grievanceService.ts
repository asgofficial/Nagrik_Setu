...
// 4. Public Safety - Ward 12
const publicSafetyDept = getDepartmentByCategory('Public Safety');
grievances.push({
  id: 'CT-KOL-2026-8004',
  title: 'Unmonitored illegal parking blocking fire hydrant & emergency lane',
  description: 'Commercial trucks & double parking completely blocking emergency vehicle access and hydrants near College Square Market.',
  category: 'Public Safety',
  severity: 'HIGH',
  latitude: 22.5738,
  longitude: 88.3655,
  landmark: 'College Square East Gate',
  createdAt: new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString(), // 3 hours ago
  status: 'REPORTED',
  isAnonymous: false,
  authorityId: publicSafetyDept.id,
  reporterId: 'user-citizen-p1',
  evidence: ['https://images.unsplash.com/photo-1544620347-c4fd4a3d5957?w=800&q=80'],
  timeline: [
    { status: 'REPORTED', note: 'Emergency access lane blocked by double parked commercial trucks.', updatedAt: new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString(), updatedBy: 'Citizen P1' },
    { status: 'AI_CLASSIFIED', note: 'AI classified as Public Safety & Nuisance. Priority: HIGH. Dispatched to Municipal Security & Police Coordination cell.', updatedAt: new Date(Date.now() - 3 * 60 * 60 * 1000 + 45 * 1000).toISOString(), updatedBy: 'AI Engine' }
  ],
  citizenConfirmations: 8,
  slaDeadline: new Date(Date.now() + 21 * 60 * 60 * 1000).toISOString(),
  isEscalated: false
});
...
grievances.push({
  id: 'CT-KOL-2026-9005',
  title: 'Frequent street harassment & lack of night patrol near bus terminal',
  description: 'Lack of lighting and security patrol near Shyambazar bus stop leading to eve-teasing and harassment of women commuters after 8 PM.',
  category: 'Harassment',
  severity: 'HIGH',
  latitude: 22.6001,
  longitude: 88.3712,
  landmark: 'Shyambazar 5-Point Crossing Bus Stand',
  createdAt: new Date(Date.now() - 5 * 60 * 60 * 1000).toISOString(), // 5 hours ago
  status: 'ASSIGNED',
  isAnonymous: true,
  authorityId: publicSafetyDept.id,
  reporterId: 'user-anonymous-h',
  evidence: [],
  timeline: [
    { status: 'REPORTED', note: 'Safety concern and harassment issue reported near bus terminal.', updatedAt: new Date(Date.now() - 5 * 60 * 60 * 1000).toISOString(), updatedBy: 'Anonymous Citizen' },
    { status: 'AI_CLASSIFIED', note: 'AI classified as Harassment & Women Safety. Priority: HIGH. Routed to Police Coordination Cell.', updatedAt: new Date(Date.now() - 5 * 60 * 60 * 1000 + 30 * 1000).toISOString(), updatedBy: 'AI Engine' },
    { status: 'ASSIGNED', note: 'Dispatched to Women Safety Mobile Patrol Unit 2 for evening coverage.', updatedAt: new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString(), updatedBy: publicSafetyDept.responsibleOfficer }
  ],
  citizenConfirmations: 12,
  slaDeadline: new Date(Date.now() + 19 * 60 * 60 * 1000).toISOString(),
  isEscalated: false
});

// 6. Transport / Traffic Signal Malfunction - Ward 7
grievances.push({
  id: 'CT-KOL-2026-9106',
  title: 'Traffic signal light stuck on green on all 4 lanes simultaneously',
  description: 'Digital traffic signal timer board malfunctioning at Bowbazar junction causing conflicting traffic movement and near collisions.',
  category: 'Transport',
  severity: 'CRITICAL',
  latitude: 22.5680,
  longitude: 88.3610,
  landmark: 'Bowbazar Central Avenue Crossing',
  createdAt: new Date(Date.now() - 1 * 60 * 60 * 1000).toISOString(), // 1 hour ago
  status: 'WORK_STARTED',
  isAnonymous: false,
  authorityId: roadDepartment.id,
  reporterId: 'user-citizen-t1',
  evidence: [],
  timeline: [
    { status: 'REPORTED', note: 'Traffic signal light stuck on green causing gridlock and danger.', updatedAt: new Date(Date.now() - 1 * 60 * 60 * 1000).toISOString(), updatedBy: 'Citizen T1' },
    { status: 'AI_CLASSIFIED', note: 'AI classified as Transport Infrastructure. Priority: CRITICAL.', updatedAt: new Date(Date.now() - 55 * 60 * 1000).toISOString(), updatedBy: 'AI Engine' },
    { status: 'WORK_STARTED', note: 'Traffic Police Sergeant on site manually operating signal panel.', updatedAt: new Date(Date.now() - 20 * 60 * 1000).toISOString(), updatedBy: 'Traffic Sergeant' }
  ],
  citizenConfirmations: 31,
  slaDeadline: new Date(Date.now() + 23 * 60 * 60 * 1000).toISOString(),
  isEscalated: false
});
...
export function getGrievancesByCategory(category?: string): Grievance[] {
  let list = [...grievances];
...
export function getRecentDataByCategory(category?: string, limit: number = 50): { grievances: Grievance[]; clusters: CivicCluster[] } {
  let matchedGrievances = [...grievances];
  let matchedClusters = [...clusters];

  if (category && category !== 'All' && category !== 'all') {
    matchedGrievances = matchedGrievances.filter(g => g.category.toLowerCase() === category.toLowerCase());
    matchedClusters = matchedClusters.filter(c => c.category.toLowerCase() === category.toLowerCase());
  }

  matchedGrievances.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  matchedClusters.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());

  return {
    grievances: matchedGrievances.slice(0, limit),
