...
export default function AccessibilitySettings() {
  ...
  const [themeMode, setThemeModeState] = useState<'bright' | 'dark'>('bright');
  const [isOpen, setIsOpen] = useState(false);
  const [readAloud, setReadAloud] = useState(false);

  // Sync theme mode on mount with document class and localStorage
  useEffect(() => {
    if (typeof window === 'undefined') return;

    const savedTheme = localStorage.getItem('nagriksetu_theme') as 'bright' | 'dark' | null;
    const systemPrefersDark = window.matchMedia('(prefers-color-scheme: dark)').matches;
    const isDarkActive = document.documentElement.classList.contains('dark');

    let initialMode: 'bright' | 'dark' = 'bright';
    if (savedTheme) {
      initialMode = savedTheme;
    } else if (isDarkActive || systemPrefersDark) {
      initialMode = 'dark';
    }
  ...
  // Keyboard shortcut listener (Escape closes menu)
  useEffect(() => {
    if (!isOpen) return;
  ...
  const setThemeMode = (mode: 'bright' | 'dark') => {
    setThemeModeState(mode);
    if (typeof document !== 'undefined') {
      // Enable smooth color transitions during toggle
      ...
      localStorage.setItem('nagriksetu_theme', mode);
  ...
  const toggleReadAloud = () => {
    ...
    if (!readAloud) {
      ...
      if (language === 'hi') utterance.text = "एक्सेसिबिलिटी सेटिंग्स पैनल खुला है। आप टेक्स्ट आकार बदल सकते हैं, हाई कंट्राস্ট चालू कर सकते हैं।";
