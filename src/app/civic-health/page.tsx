...
export default function CivicHealthPage() {
  ...
  useEffect(() => {
    if (wards.length > 0 && !wards.some(w => w.id === activeWardId)) {
  ...
  if (!activeWard) {
    return (
      <div className="max-w-6xl mx-auto px-4 py-12 text-stone-800 dark:text-stone-100 flex-1 w-full">
        <div className="rounded-3xl border border-stone-200 dark:border-stone-800 bg-white dark:bg-stone-950 p-10 shadow-xl">
          <h1 className="text-3xl font-extrabold">Civic Health Indicator Index</h1>
          <p className="mt-4 text-sm text-stone-500 dark:text-stone-400">
            No live ward data is available yet. Please ensure grievances are loaded or sign in to view live civic health analytics.
          </p>
        </div>
      </div>
    );
  ...
  const factors = activeWard ? getScoreExplanation(activeWard.healthScore) : [];

  // 1. Data format for Recharts Bar Chart (avg resolution days per ward)
  const chartLabel = activeWard ? activeWard.name.replace(/\s*\(.*\)$/, '') : 'Selected Ward';
  const barData = wards.map(w => ({
    name: w.name.replace(/\s*\(.*\)$/, ''),
  ...
  const pieData = activeWard ? [
    ...
    { name: 'Electrical/Lighting', value: activeWard.contributingFactors.electricityComplaints, color: '#f59e0b' },
    { name: 'Other Issues', value: activeWard.contributingFactors.otherComplaints, color: '#8b5cf6' }
  ] : [];
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 sm:py-12 text-stone-800 dark:text-stone-100 flex-1 w-full space-y-8">
      ...
      <div className="mb-6 border-b pb-4 flex flex-wrap justify-between items-center gap-4">
        <div>
          ...
          <p className="text-xs text-stone-500 mt-1 uppercase tracking-widest font-bold">Live ward health score computed from current grievance reports, verification rates, and resolution performance</p>
        </div>
        ...
        <div className="lg:col-span-6 space-y-6">
          <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-850 rounded-2xl p-6 shadow-xs">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-4 bg-stone-50 dark:bg-stone-950 rounded-xl border border-stone-150">
                <span className="text-[10px] font-bold text-stone-400 uppercase tracking-wider block">Associated Clusters</span>
                <span className="text-xl font-extrabold mt-1 block">{activeWard.contributingFactors.associatedClusters}</span>
              </div>
              <div className="p-4 bg-stone-50 dark:bg-stone-950 rounded-xl border border-stone-150">
        ...
        <div className="lg:col-span-6 space-y-6">
          <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-850 rounded-2xl p-6 shadow-xs flex flex-col min-h-[300px]">
            <h3 className="font-bold text-base text-stone-900 dark:text-white border-b pb-4 mb-4">
              Complaint Categories in {chartLabel}
            </h3>
