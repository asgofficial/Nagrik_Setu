...
import { Grievance, CivicCluster, GrievanceStatus, GrievanceCategory } from '@/types';
...
export default function AuthorityPortal() {
  ...
  const { grievances, clusters, updateStatus, refreshGrievances } = useApp();

  // Selected state
  const [selectedGrievanceId, setSelectedGrievanceId] = useState<string | null>(null);
  const [selectedClusterId, setSelectedClusterId] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<'grievances' | 'clusters'>('grievances');
  ...
  // Auth Guard Screen for non-officers
  if (authLoading) {
    return (
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:py-12 text-stone-900 dark:text-stone-50 space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="col-span-2 lg:col-span-1 rounded-2xl border border-stone-200 bg-white p-4 shadow-xs dark:border-stone-800 dark:bg-stone-900 text-left">
          <p className="text-[10px] font-bold uppercase tracking-wider text-stone-400">Avg Resolution</p>
          <p className="mt-1 text-3xl font-black text-stone-900 dark:text-white">3.2d</p>
          <span className="mt-1 block text-[10px] text-stone-500">SLA target &lt; 4.0d</span>
        </div>
              ...
              {filteredGrievances.length === 0 ? (
                ...
                <div className="space-y-3">
                  {filteredGrievances.map((g) => {
                    ...
                    let severityBadge = 'bg-stone-100 text-stone-600 dark:bg-stone-800 dark:text-stone-300';
                    if (g.severity === 'HIGH') severityBadge = 'bg-amber-100 text-amber-800 dark:bg-amber-950/50 dark:text-amber-300';
                    if (g.severity === 'CRITICAL') severityBadge = 'bg-red-100 text-red-800 dark:bg-red-950/50 dark:text-red-300';
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex flex-wrap items-center gap-1.5 text-[10px] font-bold">
                            ...
                            <span className={`px-2 py-0.5 rounded uppercase tracking-wider ${severityBadge}`}>
                              {g.severity}
                            </span>
                          ...
                          <span className={`text-[10px] font-extrabold uppercase px-2 py-0.5 rounded ${
                            g.status === 'CITIZEN_VERIFIED' || g.status === 'CLOSED'
              ...
              <div className="pt-4 border-t border-stone-100 dark:border-stone-800 space-y-3">
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                  {selectedGrievance.timeline.map((item, idx) => (
                    <div key={idx} className="rounded-lg bg-stone-50 p-2.5 text-[11px] text-stone-600 dark:bg-stone-950 dark:text-stone-400">
            ...
            <div className="rounded-2xl border border-stone-200 bg-white p-6 shadow-xs dark:border-stone-800 dark:bg-stone-900 text-left space-y-5">
              ...
              <button
                onClick={() => {
                  ...
                  alert(`Dispatched unified repair crew for ${selectedCluster.title}!`);
                }}
