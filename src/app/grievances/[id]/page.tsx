...
import { Sparkles, ArrowLeft, Check, X, Shield, Clock, Users, MapPin, AlertCircle, AlertTriangle, CheckCircle2, Send, Loader2, Camera } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 sm:py-12 text-stone-800 dark:text-stone-100 flex-1 w-full space-y-8">
      ...
      <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-850 rounded-2xl p-6 shadow-xs relative">
        ...
        {/* Geotagged Live Evidence Section */}
        {grievance.evidence && grievance.evidence.length > 0 && (
          <div className="mt-6 pt-6 border-t border-stone-100 dark:border-stone-800 space-y-3">
            <h4 className="text-xs font-bold text-stone-700 dark:text-stone-300 flex items-center gap-1.5 uppercase tracking-wider">
              ...
              <span>Verified Live Photographic Evidence</span>
            </h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {grievance.evidence.map((imgUrl, idx) => (
                <div key={idx} className="relative rounded-xl overflow-hidden border border-stone-200 dark:border-stone-800 shadow-xs group bg-black">
                  <img
                    src={imgUrl}
                    alt={`Grievance Evidence ${idx + 1}`}
                    className="w-full h-48 sm:h-56 object-cover"
                  />
                  <div className="absolute bottom-2 left-2 flex items-center gap-1 bg-black/80 backdrop-blur-md px-2 py-0.5 rounded text-[10px] text-white font-mono">
                    <MapPin className="h-3 w-3 text-orange-400" />
                    <span>Live Geotagged Photo</span>
                  </div>
                </div>
              ))}
            </div>
