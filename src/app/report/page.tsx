...
export default function ReportIssue() {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (typeof window !== 'undefined' && navigator.geolocation && !latitude) {
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-850 rounded-2xl p-6 shadow-xs">
            <h3 className="font-bold text-sm text-stone-900 dark:text-white mb-2">Upload Evidence</h3>
            <p className="text-[11px] text-stone-400 leading-relaxed mb-4">Evidence helps authorities prioritize and confirm repairs. GPS metadata will be stripped automatically.</p>
            
            <div className="border-2 border-dashed border-stone-200 dark:border-stone-800 rounded-xl p-6 text-center hover:bg-stone-50 dark:hover:bg-stone-950 transition relative">
              <input
                type="file"
                accept="image/*"
                capture="environment"
              ...
              {isUploading ? (
                <div className="flex flex-col items-center gap-1.5">
                  ...
                  <span className="text-xs font-semibold text-stone-500">Processing file & stripping metadata...</span>
                </div>
              ) : evidence.length > 0 ? (
                <div className="space-y-3">
                  ...
                  <div className="flex flex-col text-left gap-1">
                    <span className="text-[10px] font-bold text-emerald-600 block">✓ Photo captured directly from phone camera.</span>
                    <span className="text-[9px] font-bold text-purple-650 bg-purple-50 dark:bg-purple-950/20 px-2 py-0.5 rounded w-fit">
                      📍 Geotagged: {latitude ? latitude.toFixed(4) : '--'}° N, {longitude ? longitude.toFixed(4) : '--'}° E
                    </span>
                    <span className="text-[8px] text-stone-400">EXIF metadata stripped; location loaded automatically.</span>
                  </div>
                </div>
              ) : (
                <div className="flex flex-col items-center gap-2 text-stone-500">
                  <Camera className="h-8 w-8 text-stone-400" />
                  <span className="text-xs font-bold text-stone-750 dark:text-stone-200">Drag or select photograph</span>
                  <span className="text-[10px] text-stone-400">PNG, JPG up to 10MB</span>
                </div>
              )}
            </div>
          </div>
